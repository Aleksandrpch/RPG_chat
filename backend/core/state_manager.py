...
# 1. РАБОТА С ПЕРСОНАЖЕМ
    ...
    async def save_character_state(self, state: CharacterState):
        async with self.pool.acquire() as conn:
            await conn.execute( """
                ...
                """, state.character_id, state.world_id, json.dumps(state.model_dump())
                )
    # 2. Работа с миром 
    ...
    async def save_world_state(self,state: WorldState): 
        async with self.pool.acquire() as conn: 
            ...
            """, state.world_id, json.dumps(state.model_dump())
            )
    ...
    async def add_event(self, event: WorldEvent):
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                """,
                event.world_id, event.character_id, event.event_type,
